...
@app.route('/venues')
def venues():
  # areas return a state city pair
  areas = Venue.query.with_entities(
        Venue.state, Venue.city).group_by(Venue.state, Venue.city).all()
  # data is a list of dictionary, each dictionary represents a state ehich contains all corresponding venues
  data = []
  # the page doesn't show num_upcoming_shows
...
def delete_venue(venue_id):
  ...
  error = False
  try:
      #use db.session.query(Venue) instead of Venue.query
      #to avoid 'Object already attached to session' error
      #also delete related shows
